/* detector_general.c
** Raptor Ninox-640 Infrared detector library : general routines
*/
/**
 * Error and Log handlers for the Raptor Ninox-640 Infrared detector library.
...
/* data types */
/**
 * Data type holding local data to detector_general. This consists of the following:
 * <dl>
 * <dt>Mutex</dt> <dd>Optionally compiled mutex locking over sending commands and receiving a reply.</dd>
 ...
 * 		This is set using Detector_General_Set_Log_Filter_Level.
 * 		Detector_Log_General_Filter_Level_Absolute and Detector_General_Log_Filter_Level_Bitwise 
 *              test it against message levels to determine whether to log messages.</dd>
 * </dl>
 * @see #Detector_General_Log
 * @see #Detector_General_Set_Log_Filter_Level
 * @see #Detector_General_Log_Filter_Level_Absolute
 * @see #Detector_General_Log_Filter_Level_Bitwise
 */
 ...
 * @see #DETECTOR_GENERAL_ERROR_STRING_LENGTH
 */
static char General_Error_String[DETECTOR_GENERAL_ERROR_STRING_LENGTH] = "";
...
void Detector_General_Error(void)
{
	...
	if(General_Error_Number != 0)
	{
		...
		fprintf(stderr,"%s Detector_General:Error(%d) : %s\n",time_string,General_Error_Number,
			General_Error_String);
	}
	if(!found)
	{
		fprintf(stderr,"%s Detector_General_Error:An unknown error has occured.\n",time_string);
	}
}
...
void Detector_General_Error_To_String(char *error_string)
{
	...
	{
		Detector_General_Get_Current_Time_String(time_string,32);
		sprintf(error_string+strlen(error_string),"%s Detector_General:Error(%d) : %s\n",time_string,
			General_Error_Number,General_Error_String);
	}
	if(strlen(error_string) == 0)
	{
		sprintf(error_string,"%s Error:Detector_General:Error not found\n",time_string);
	}
}
 ...
 * @see #General_Error_Number
 */
int Detector_General_Get_Error_Number(void)
{
 ...
 * @see #DETECTOR_GENERAL_ONE_MILLISECOND_NS
 */
void Detector_General_Get_Current_Time_String(char *time_string,int string_length)
{
	...
	sprintf(millsecond_string,"%03ld",(current_time.tv_nsec/DETECTOR_GENERAL_ONE_MILLISECOND_NS));
 ...
 * Detector_General_Log is then called to handle the log message.
 ...
 * @see #LOG_BUFF_LENGTH
 */
void Detector_General_Log_Format(int level,char *format,...)
{
...
/* call the log routine to log the results */
	Detector_General_Log(level,buff);
}
 ...
 * @see #General_Data
 */
void Detector_General_Log(int level,char *string)
{
...
/**
 * Routine to set the General_Data.Log_Handler used by Detector_General_Log.
 * @param log_fn A function pointer to a suitable handler.
 * @see #General_Data
 * @see #Detector_General_Log
 */
void Detector_General_Set_Log_Handler_Function(void (*log_fn)(int level,char *string))
{
	General_Data.Log_Handler = log_fn;
}

/**
 * Routine to set the General_Data.Log_Filter used by Detector_General_Log.
 * @param log_fn A function pointer to a suitable filter function.
 * @see #General_Data
 * @see #Detector_General_Log
 */
void Detector_General_Set_Log_Filter_Function(int (*filter_fn)(int level,char *string))
{
 ...
 * Detector_General_Get_Current_Time_String.
 * @param level The log level for this message.
 * @param string The log message to be logged. 
 * @see #Detector_General_Get_Current_Time_String
 */
void Detector_General_Log_Handler_Stdout(int level,char *string)
{
	char time_string[32];
 ...
 * @see #General_Data
 */
void Detector_General_Set_Log_Filter_Level(int level)
{
 ...
 * @see #General_Data
 */
int Detector_General_Log_Filter_Level_Absolute(int level,char *string)
{
 ...
 * @see #General_Data
 */
int Detector_General_Log_Filter_Level_Bitwise(int level,char *string)
{
 ...
 * @see #General_Data
 */
int Detector_General_Mutex_Lock(void)
{
	int error_number;
	...
	if(error_number != 0)
	{
		...
		sprintf(General_Error_String,"Detector_General_Mutex_Lock:Mutex lock failed '%d'.",error_number);
		return FALSE;
	}
	return TRUE;
}
 ...
 * @see #General_Data
 */
int Detector_General_Mutex_Unlock(void)
{
	int error_number;
	...
	if(error_number != 0)
	{
		...
		sprintf(General_Error_String,"Detector_General_Mutex_Unlock:Mutex unlock failed '%d'.",
			error_number);
		return FALSE;
	}
	return TRUE;
