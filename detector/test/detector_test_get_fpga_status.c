/* detector_test_get_fpga_status.c */
/**
 * Test program to use the XCLIB serial interface to the Raptor Ninox-640 to query the FPGA status.
...
int main(int argc, char *argv[])
{
	...
	/* parse arguments */
	fprintf(stdout,"detector_test_get_fpga_status : Parsing Arguments.\n");
	...
	fprintf(stdout,"detector_test_get_fpga_status : Initialising Detector library.\n");
	...
	if((status & DETECTOR_SERIAL_FPGA_CTRL_TEC_ENABLED) > 0)
		...
		fprintf(stdout,"TEC is disabled.\n");
	if((status & DETECTOR_SERIAL_FPGA_CTRL_AUTO_EXPOSURE_ENABLED) > 0)
		fprintf(stdout,"Auto Exposure is enabled.\n");
	else
		fprintf(stdout,"Auto Exposure is disabled.\n");
	if((status & DETECTOR_SERIAL_FPGA_CTRL_FAN_ENABLED) > 0)
		fprintf(stdout,"Fan is enabled.\n");
	else
		fprintf(stdout,"Fan is disabled.\n");
	if((status & DETECTOR_SERIAL_FPGA_CTRL_INVERT_VIDEO_ENABLED) > 0)
		fprintf(stdout,"Invert Video is enabled.\n");
	else
		fprintf(stdout,"Invert Video is disabled.\n");
	if((status & DETECTOR_SERIAL_FPGA_CTRL_HORIZONTAL_FLIP_ENABLED) > 0)
		fprintf(stdout,"Horizontal Flip is enabled.\n");
	else
		fprintf(stdout,"Horizontal Flip is disabled.\n");
...
static void Help(void)
{
	...
	fprintf(stdout,"detector_test_get_system_status [-coadd[_exposure_length] <ms>][-fmt[_directory] <dir>]\n");
	...
	fprintf(stdout,"The -coadd_exposure_length / -fmt_directory arguments are needed to construct a valid '.fmt. filename, which is needed to open a conenction to the XCLIB library.\n");
}
