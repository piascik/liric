/* detector_test_tec_setpoint_set.c */
/**
 * Test program to test setting the TEC set-point temperature on the Raptor Ninox-640 camera head.
...
/**
 * The passed in set-point temperature to configure the TEC set-point temperature to, in degrees centigrade.
 */
static double Setpoint_Temperature_C = 0.0;
 ...
 * @see ../cdocs/detector_temperature.html#Detector_Temperature_Set_TEC_Setpoint
 */
int main(int argc, char *argv[])
{
	...
	/* parse arguments */
	fprintf(stdout,"detector_test_tec_setpoint_set : Parsing Arguments.\n");
	...
	fprintf(stdout,"detector_test_tec_setpoint_set : Initialising Detector library.\n");
	...
	/* Set the camera head TEC set-point. We must have already initialised the serial device, which causes the 
	** manufacturers data to be read (including ADC/DAC calibration values), and the temperature software
	** to be initialised. */
	fprintf(stdout,
		"detector_test_tec_setpoint_set : Setting TEC set-point temperature to %.3f degrees Centigrade.\n",
		Setpoint_Temperature_C);
	if(!Detector_Temperature_Set_TEC_Setpoint(Setpoint_Temperature_C))
	{
		Detector_General_Error();
		Detector_Setup_Close();
		return 3;
	...
	fprintf(stdout,"detector_test_tec_setpoint_set : Finished.\n");		
	return 0;
}
...
static int Parse_Arguments(int argc, char *argv[])
{
	...
	for(i=1;i<argc;i++)
	{
		...
		else if((strcmp(argv[i],"-t")==0)||(strcmp(argv[i],"-temperature")==0))
		{
			if((i+1)<argc)
			{
				retval = sscanf(argv[i+1],"%lf",&Setpoint_Temperature_C );
				if(retval != 1)
				{
					fprintf(stderr,"Parse_Arguments:Failed to parse set-point temperature %s.\n",argv[i+1]);
					return FALSE;
				}
				i++;
			}
			...
			{
				fprintf(stderr,"Parse_Arguments:-temperature requires a TEC set-point temperature in degrees centigrade.\n");
				return FALSE;
			}
		}
		else
...
static void Help(void)
{
	fprintf(stdout,"Detector Test Setting the TEC set-point temperature:Help.\n");
	fprintf(stdout,"This program tests setting the TEC set-point temperature on the Raptor Ninox-640 camera head.\n");
	fprintf(stdout,"detector_test_tec_setpoint_set -t[emperature] <deg C>\n");
	fprintf(stdout,"\t[-coadd[_exposure_length] <ms>][-fmt[_directory] <dir>]\n");
	fprintf(stdout,"\t[-help][-l[og_level <0..5>].\n");
	fprintf(stdout,"The set-point temperature is specified in degrees centigrade.\n");
