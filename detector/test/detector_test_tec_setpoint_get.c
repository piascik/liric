/* detector_test_tec_setpoint_get.c */
/**
 * Test program to test retrieving the TEC set-point temperature from the Raptor Ninox-640 camera head.
 ...
 * @see ../cdocs/detector_temperature.html#Detector_Temperature_Get_TEC_Setpoint
 */
int main(int argc, char *argv[])
{
	...
	/* parse arguments */
	fprintf(stdout,"detector_test_tec_setpoint_get : Parsing Arguments.\n");
	...
	fprintf(stdout,"detector_test_tec_setpoint_get : Initialising Detector library.\n");
	...
	** to be initialised. */
	if(!Detector_Temperature_Get_TEC_Setpoint(&tec_setpoint))
	{
		Detector_General_Error();
		Detector_Setup_Close();
		return 3;
	}		
	fprintf(stdout,"detector_test_tec_setpoint_get : TEC set-point temperature is %.3f degrees Centigrade.\n",
		tec_setpoint);
	...
	fprintf(stdout,"detector_test_tec_setpoint_get : Finished.\n");		
	return 0;
}
...
static void Help(void)
{
	fprintf(stdout,"Detector Test Getting the TEC set-point temperature:Help.\n");
	fprintf(stdout,"This program tests retrieving the TEC set-point temperature from the Raptor Ninox-640 camera head.\n");
	fprintf(stdout,"detector_test_tec_setpoint_get [-coadd[_exposure_length] <ms>][-fmt[_directory] <dir>]\n");
