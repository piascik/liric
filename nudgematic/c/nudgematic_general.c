 ...
 * 		This is set using Nudgematic_General_Set_Log_Filter_Level.
 * 		Nudgematic_General_Log_General_Filter_Level_Absolute and Nudgematic_General_Log_Filter_Level_Bitwise 
 *              test it against message levels to determine whether to log messages.</dd>
 * </dl>
 * @see #Nudgematic_General_Log
 * @see #Nudgematic_General_Set_Log_Filter_Level
 * @see #Nudgematic_General_Log_Filter_Level_Absolute
 * @see #Nudgematic_General_Log_Filter_Level_Bitwise
 */
 ...
 * @see #NUDGEMATIC_GENERAL_ERROR_STRING_LENGTH
 */
static char General_Error_String[NUDGEMATIC_GENERAL_ERROR_STRING_LENGTH] = "";
...
void Nudgematic_General_Error(void)
{
	...
	if(General_Error_Number != 0)
	{
		...
		fprintf(stderr,"%s Nudgematic_General:Error(%d) : %s\n",time_string,General_Error_Number,
			General_Error_String);
	}
	if(!found)
	{
		fprintf(stderr,"%s Nudgematic_General_Error:An unknown error has occured.\n",time_string);
	}
}
 ...
 * @see #General_Error_Number
 */
int Nudgematic_General_Get_Error_Number(void)
{
 ...
 * @see #NUDGEMATIC_GENERAL_ONE_MILLISECOND_NS
 */
void Nudgematic_General_Get_Current_Time_String(char *time_string,int string_length)
{
	...
	sprintf(millsecond_string,"%03ld",(current_time.tv_nsec/NUDGEMATIC_GENERAL_ONE_MILLISECOND_NS));
 ...
 * Nudgematic_General_Log is then called to handle the log message.
 ...
 * @see #LOG_BUFF_LENGTH
 */
void Nudgematic_General_Log_Format(int level,char *format,...)
{
...
/* call the log routine to log the results */
	Nudgematic_General_Log(level,buff);
}
 ...
 * @see #General_Data
 */
void Nudgematic_General_Log(int level,char *string)
{
...
/**
 * Routine to set the General_Data.Log_Handler used by Nudgematic_General_Log.
 * @param log_fn A function pointer to a suitable handler.
 * @see #General_Data
 * @see #Nudgematic_General_Log
 */
void Nudgematic_General_Set_Log_Handler_Function(void (*log_fn)(int level,char *string))
{
	General_Data.Log_Handler = log_fn;
}

/**
 * Routine to set the General_Data.Log_Filter used by Nudgematic_General_Log.
 * @param log_fn A function pointer to a suitable filter function.
 * @see #General_Data
 * @see #udgematic_General_Log
 */
void Nudgematic_General_Set_Log_Filter_Function(int (*filter_fn)(int level,char *string))
{
 ...
 * Nudgematic_General_Get_Current_Time_String.
 * @param level The log level for this message.
 * @param string The log message to be logged. 
 * @see #Nudgematic_General_Get_Current_Time_String
 */
void Nudgematic_General_Log_Handler_Stdout(int level,char *string)
{
	char time_string[32];
 ...
 * @see #General_Data
 */
void Nudgematic_General_Set_Log_Filter_Level(int level)
{
 ...
 * @see #General_Data
 */
int Nudgematic_General_Log_Filter_Level_Absolute(int level,char *string)
{
 ...
 * @see #General_Data
 */
int Nudgematic_General_Log_Filter_Level_Bitwise(int level,char *string)
{
 ...
 * @see #General_Data
 */
int Nudgematic_General_Mutex_Lock(void)
{
	int error_number;
	...
	if(error_number != 0)
	{
		...
		sprintf(General_Error_String,"Nudgematic_General_Mutex_Lock:Mutex lock failed '%d'.",error_number);
		return FALSE;
	}
	return TRUE;
}
 ...
 * @see #General_Data
 */
int Nudgematic_General_Mutex_Unlock(void)
{
	int error_number;
	...
	if(error_number != 0)
	{
		...
		sprintf(General_Error_String,"Nudgematic_General_Mutex_Unlock:Mutex unlock failed '%d'.",
			error_number);
		return FALSE;
	}
	return TRUE;
