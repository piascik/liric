...
/**
 * Test program to try and get the inputs from the USB-PIO BMCM OR8 I/O card.
 */
/**
 ...
 * @see ../cdocs/usb_pio_command.html#USB_PIO_Command_Inputs_Get
 */
int main(int argc, char *argv[])
{
	...
	/* parse arguments */
	fprintf(stdout,"usb_pio_test_get_inputs : Parsing Arguments.\n");
	...
	USB_PIO_General_Set_Log_Handler_Function(USB_PIO_General_Log_Handler_Stdout);
         /* open device */
	USB_PIO_General_Log(LOG_VERBOSITY_TERSE,"usb_pio_test_get_inputs : Connecting to controller.");
	...
	fprintf(stdout,"usb_pio_test_get_inputs:Inputs were %#x.\n",inputs);
	fprintf(stdout,"usb_pio_test_get_inputs:Closing connection.\n");
...
static void Help(void)
{
	fprintf(stdout,"Test USB PIO Inputs:Help.\n");
	fprintf(stdout,"This program gets the current input state from the USB-PIO BMCM OR8 I/O board.\n");
	fprintf(stdout,"usb_pio_test_get_inputs -d[evice_name] <USB device> [-help]\n");
