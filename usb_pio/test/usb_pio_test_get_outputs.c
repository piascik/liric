...
/**
 * Test program to try and get the current settings of the digital outputs from the USB-PIO BMCM OR8 I/O card.
 */
/**
 ...
 * @see ../cdocs/usb_pio_command.html#USB_PIO_Command_Outputs_Get
 */
int main(int argc, char *argv[])
{
	...
	/* parse arguments */
	fprintf(stdout,"usb_pio_test_get_outputs : Parsing Arguments.\n");
	...
	USB_PIO_General_Set_Log_Handler_Function(USB_PIO_General_Log_Handler_Stdout);
         /* open device */
	USB_PIO_General_Log(LOG_VERBOSITY_TERSE,"usb_pio_test_get_outputs : Connecting to controller.");
	...
	fprintf(stdout,"usb_pio_test_get_outputs:Outputs were %#x.\n",outputs);
	fprintf(stdout,"usb_pio_test_get_outputs:Closing connection.\n");
...
static void Help(void)
{
	fprintf(stdout,"Test USB PIO Get Outputs:Help.\n");
	fprintf(stdout,"This program gets the current state of the digital outputs from the USB-PIO BMCM OR8 I/O board.\n");
